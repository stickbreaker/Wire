/*  Master/Slave I2C test set for Modified Wire Library
  (c) Chuck Todd <ctodd@cableone.net>
...
Example I2C Master / Slave combination  Sends and receives a datapacket of 8 bytes,
  ...
  To test the timeout function, ground SCL and/or SDA this will put the I2C bus into a halt,
  The standard Wire Library will hang forEVER waiting for the bus to clear.  My modifications
  cause the functions to return error code 4 (other Error),except requestFrom() which returns zero, if lastError() is called the actual tw_error code is returned.  do a search on compat\twi.h for the meaning of each value.
...
void onRequestEvent(void){ // will only work with my modified WIRE library because of the multiple Wire.write()'s
 /*  pointer type casting test  works
   uint8_t * bp=(uint8_t*)&fnum;
   Wire.write(bp,sizeof fnum);
   */
   
   /* inline type casting  and this works too!
   Wire.write((uint8_t*)&fnum,sizeof fnum);
   */
...
void scan(){
Serial.println(F(" Scanning I2C Addresses in HEX"));
uint8_t cnt=0;
  ...
  if (Serial.available()){ // keypressed from Serial Monitor
	...
	switch (ch){
	  ...
	  if (digitalRead(A0)==LOW){ // automated send/ receive update so I could monitor SLAVE. Ground A0 to activate
		  if(me=SLAVE_ADDRESS_1)delay(200);
		  else delay(1000);
 		  foo = foo +0.1;
          fnum = fnum +2;
          Wire.beginTransmission(other);
